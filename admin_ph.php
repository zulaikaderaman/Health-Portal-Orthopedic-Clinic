...
<div class="w3-content" style="max-width:2000px;margin-top:46px">
    ...
    <div class="main">
        ...
        <form id='update' action='admin_phdelete.php' method="post">
            <?php

            $query = "SELECT * FROM physicians "; //have to do in this way because the userid hold a varchar
            ...
            while($row= mysqli_fetch_array($result)){
                $ph_personalcode=$row["ph_personalcode"];

                echo'
             <tr>
             <td><img src="image/physician/'.$row['ph_profilepic'].'" style="width:100px;height:87px;border-radius:25px;"</td>
             <td>'.$row['ph_name'].'</td>
             <td>'.$row['ph_address'].'</td>
             <td>'.$row['ph_email'].'</td>
             <td>'.$row['ph_phonenum'].'</td>
             <td>'.$row['ph_qualification'].'</td>
             <td><a class="btn btn-danger btn-sm" href="admin_phdelete.php?id='.$row["ph_personalcode"].'"><i class="fa fa-trash"> Delete</i></a></td>
             </tr>';
